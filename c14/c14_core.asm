         ...
         core_code_seg_sel     equ  0x38    ;内核代码段选择子 RPL:00，对应描述符DPL = 00
         core_data_seg_sel     equ  0x30    ;内核数据段选择子 RPL:00 ，对应描述符DPL = 00
         sys_routine_seg_sel   equ  0x28    ;系统公共例程代码段的选择子 RPL:00，对应描述符DPL = 00
         video_ram_seg_sel     equ  0x20    ;视频显示缓冲区的段选择子  RPL:00，对应描述符DPL = 00
         core_stack_seg_sel    equ  0x18    ;内核堆栈段选择子         RPL:00，对应描述符DPL = 00
         mem_0_4_gb_seg_sel    equ  0x08    ;整个0-4GB内存的段的选择子 RPL:00，对应描述符DPL = 00
...
start:
         ...
         mov edi,salt                       ;C-SALT表的起始位置 C代表kernal
         mov ecx,salt_items                 ;C-SALT表的条目数量 C代表kernal
  .b3:
         push ecx                           ;保存EXC，EXC作为循环的计数器                      
         mov eax,[edi+256]                  ;该条目表示入口点的32位偏移地址 
         mov bx,[edi+260]                   ;该条目表示入口点的段选择子 
...
;-------------------------------------------------------------------------------
         ...
         call sys_routine_seg_sel:set_up_gdt_descriptor   ;在GDT当中，是在GDT当中安装新的描述符
;-------------------------------------------------------------------------------
                                            ;在GDT内安装一个新的描述符
                                            ;输入：EDX:EAX=描述符 
                                            ;输出：CX=描述符的选择子

         mov [edi+260],cx                   ;将返回的门描述符选择子回填（偏移不用填了，调用门调用语句的偏移地址是不用的）
         ;mov dword [edi+256],0x00000000    ;去掉注释，能正常运行。该条目表示入口点的32位偏移地址 调用门调用语句的偏移地址是不用的
         ...
         ;TODO:
         ;mov ebx,message_2                 ;测试CPL不改变的情况
         ;jmp far [salt_1+256]              ;通过门显示信息(偏移量将被忽略) 
...
;===============================================================================
SECTION test_code_ring3 vstart=0
;-------------------------------------------------------------------------------
       mov eax,32
       mov eax,32
       mov eax,32
