...
alloc_inst_a_page:                          ;分配一个页，并安装在当前活动的
  ...
  .b1:
         ...
         mov esi,ebx
         shr esi,10
         and esi,0x003ff000                 ;只保留中间10位，两边清楚
         or esi,0xffc00000                  ;将高12位改成全1，从而访问页目录表的最后一项（指向自己）。得到该页表的线性地址
...
start:
  ...
  .b3:                                      ;其余的页表项置为无效
         ...
         shl esi,2                          ;目录索引乘以4就是偏移  esi里面放的是0x800
         mov dword [es:ebx+esi],0x00021003  ;写入目录项（页表的物理地址和属性）
                                            ;TODO:测试这里：目标单元的线性地址为0xFFFFF800
